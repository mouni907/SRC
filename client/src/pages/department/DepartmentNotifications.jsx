...
import { getDepartmentMeta } from './departmentData';
...
export default function DepartmentNotifications() {
  ...
  const unreadCount = departmentNotifications.filter((notification) => !notification.read).length;
  const filteredNotifications = departmentNotifications.filter((notification) => {
  ...
  const openNotification = (notification) => {
    ...
    navigate(notification.type === 'certificate_ready' ? '/department/reports' : '/department/requests');
  };
    ...
    <div className="space-y-6">
      <section className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-blue-50 text-blue-600"><Bell className="h-5 w-5" /></div>
          <div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-blue-600">{meta.label} activity center</p><h2 className="mt-1 text-xl font-bold text-slate-900">Notifications</h2><p className="mt-1 text-xs text-slate-500">Stay updated with {meta.label} clearance activity.</p></div>
        </div>
